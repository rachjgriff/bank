...
describe BankBalance do
  ...
  describe '#record_transaction' do
    ...
    it 'Stores a single transaction in transaction history' do
      allow(bank_transaction).to receive(:deposit_transaction)
      allow(bank_transaction).to receive(:transaction) { { :date => "10/01/2012", :credit => "1000.00", :debit => "", :balance => "1000.00" } }
    ...
    #   allow(bank_balance).to receive(:bank_transaction) { { :date => "10-01-2012", :credit => "1000.00", :debit => "", :balance => "1000.00" } }
    #   bank_account.record_transaction
    #
    #   allow(bank_balance).to receive(:bank_transaction) { { :date => "13-01-2012", :credit => "2000.00", :debit => "", :balance => "3000.00" } }
    #   bank_account.record_transaction
    #
    #   allow(bank_balance).to receive(:bank_transaction) { { :date => "14-01-2012", :credit => "", :debit => "500.00", :balance => "2500.00" } }
    #   bank_account.record_transaction
    #
...
# @bank_transcation_double = double('bank_transaction', date: '', credit: '', debit: '', balance: '')
